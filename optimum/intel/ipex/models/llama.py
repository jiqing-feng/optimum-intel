...
class IPEXLlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.rope_base = self.rope_theta
        self.pos_embd_dim = self.head_dim
        self.ipex_rope = ApplyRotaryEmbedding(
...
class IPEXLlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, distributed: bool = False):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = IPEXLlamaAttention(config=config)
        ...
        if not distributed:
            self.mha_linear_add = linearAdd(self.self_attn.o_proj)
            self.mlp_linear_add = linearAdd(self.mlp.down_proj)

        self.linear_silu_mul = linear2SiluMul(self.mlp.gate_proj, self.mlp.up_proj)
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["IPEXLlamaDecoderLayer"]
...
class LlamaModel(LlamaPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`IPEXLlamaDecoderLayer`]
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList([IPEXLlamaDecoderLayer(config) for _ in range(config.num_hidden_layers)])
